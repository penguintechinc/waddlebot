...
- **hub_module/**: Community portal with authentication, dashboard, and direct routing
  - **Software Discovery**: Git repository scanning and dependency tracking (Phase 17)
  - **Service Discovery**: Microservice health monitoring dashboard (Phase 17)
...
- **Task agents MUST write temporary documentation to `/tmp/`** (not within the repo)
- Implementation summaries, checklists, artifacts, progress reports → `/tmp/waddlebot-{task-name}.md`
- Test results and validation reports → `/tmp/` (not in repo)
- Only create permanent `.md` files in `docs/` if the content provides lasting reference value
- Quick references and API docs belong WITH their module (e.g., `module/README.md`)
...
**Commit**: `feat(superadmin): Add Software & Services discovery features`

**Software Discovery**:
- Git repository management (`/superadmin/software-discovery`)
- Multi-provider support: GitHub, GitLab, Bitbucket, Azure DevOps
- Dependency extraction: npm, pip, go, rust, java/maven
- Token encryption with AES-256-GCM
- Scan status tracking and dependency visualization

**Service Discovery**:
- Microservice health monitoring (`/superadmin/services`)
- 39 default WaddleBot services auto-populated from docker-compose
- Health check polling with 5-second timeout
- Response time measurement and uptime tracking
- Service dependency graph and event logging
- Category-based organization: infrastructure, core, triggers, actions, processing, admin

**Database Schema**:
- `services` table (18 columns): health monitoring with JSONB for dependencies/events
- `software_repositories` table (12 columns): git repo management with encryption support

**Testing**: All 34 tests passing on local cluster (Docker Compose)
