#!/usr/bin/env bash
################################################################################
# WaddleBot YouTube Action Module API Test Script
#
# Tests all REST API endpoints of the YouTube Action Module with comprehensive
# validation, plus gRPC connectivity checks.
#
# Usage:
#   ./test-api.sh                       # Test default localhost:8000
#   YOUTUBE_URL=http://host:port ./test-api.sh
#   ./test-api.sh --help
#
# Exit codes:
#   0 - All tests passed
...
#   YOUTUBE_URL    - Base URL for YouTube module (default: http://localhost:8000)
#   GRPC_PORT      - gRPC port for YouTube module (default: 50054)
...
# Configuration
YOUTUBE_URL="${YOUTUBE_URL:-http://localhost:8000}"
...
EXAMPLES:
    ...
    YOUTUBE_URL=http://youtube.example.com:8000 ./test-api.sh
...
make_request() {
    ...
    local url="${YOUTUBE_URL}${endpoint}"
    local curl_opts=(-s -w "\n%{http_code}" -X "$method")
...
test_health_endpoint() {
    ...
    validate_json "$response" ".module" "youtube_action_module" || {
...
test_send_comment_endpoint() {
    print_test "POST /api/v1/youtube/comments - Post comment"
    ((TESTS_RUN++))
...
EOF
    ...
    local response
    response=$(make_request POST "/api/v1/youtube/comments" "$test_request" 200) || {
        print_fail "POST /api/v1/youtube/comments - Request failed"
        return 1
    }

    validate_json "$response" "" "" || {
        print_fail "POST /api/v1/youtube/comments - Invalid JSON response"
        return 1
    }

    print_pass "POST /api/v1/youtube/comments - Post comment working"
    return 0
}

test_send_chat_message_endpoint() {
    print_test "POST /api/v1/youtube/chat - Send chat message"
    ((TESTS_RUN++))
...
EOF
    ...
    local response
    response=$(make_request POST "/api/v1/youtube/chat" "$test_request" 200) || {
        print_fail "POST /api/v1/youtube/chat - Request failed"
        return 1
    }

    validate_json "$response" "" "" || {
        print_fail "POST /api/v1/youtube/chat - Invalid JSON response"
        return 1
    }

    print_pass "POST /api/v1/youtube/chat - Send chat message working"
    return 0
}

test_like_video_endpoint() {
    print_test "POST /api/v1/youtube/like - Like video"
    ((TESTS_RUN++))
...
EOF
    ...
    local response
    response=$(make_request POST "/api/v1/youtube/like" "$test_request" 200) || {
        print_fail "POST /api/v1/youtube/like - Request failed"
        return 1
    }

    validate_json "$response" "" "" || {
        print_fail "POST /api/v1/youtube/like - Invalid JSON response"
        return 1
    }

    print_pass "POST /api/v1/youtube/like - Like video working"
    return 0
}
...
main() {
    ...
    print_header "WaddleBot YouTube Action Module API Tests"
    ...
    if ! curl -s -f -o /dev/null "$YOUTUBE_URL/health"; then
        print_error "Cannot connect to YouTube module at $YOUTUBE_URL"
        print_error "Please ensure the YouTube module is running and accessible"
        exit 1
    fi
    echo -e "${GREEN}✓${NC} YouTube module is accessible"
    ...
    print_header "YouTube Action Endpoints"
    test_send_comment_endpoint || true
    test_send_chat_message_endpoint || true
    test_like_video_endpoint || true
