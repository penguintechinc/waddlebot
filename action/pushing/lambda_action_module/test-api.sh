#!/usr/bin/env bash
################################################################################
# WaddleBot Lambda Action Module API Test Script
#
# Tests all REST API endpoints of the Lambda Action Module with comprehensive
# validation, plus gRPC connectivity checks.
#
# Usage:
#   ./test-api.sh                       # Test default localhost:8000
#   LAMBDA_URL=http://host:port ./test-api.sh
#   ./test-api.sh --help
#
# Exit codes:
#   0 - All tests passed
...
#   LAMBDA_URL     - Base URL for Lambda module (default: http://localhost:8000)
#   GRPC_PORT      - gRPC port for Lambda module (default: 50060)
...
# Configuration
LAMBDA_URL="${LAMBDA_URL:-http://localhost:8000}"
GRPC_PORT="${GRPC_PORT:-50060}"
API_KEY="${API_KEY:-}"
VERBOSE="${VERBOSE:-0}"
...
EXAMPLES:
    ...
    LAMBDA_URL=http://lambda.example.com:8000 ./test-api.sh
...
test_health_endpoint() {
    ...
    validate_json "$response" ".module" "lambda_action_module" || {
...
test_invoke_function_endpoint() {
    print_test "POST /api/v1/lambda/invoke - Invoke Lambda function"
    ((TESTS_RUN++))
...
EOF
    ...
    local response
    response=$(make_request POST "/api/v1/lambda/invoke" "$test_request" 200) || {
        print_fail "POST /api/v1/lambda/invoke - Request failed"
        return 1
    }

    validate_json "$response" "" "" || {
        print_fail "POST /api/v1/lambda/invoke - Invalid JSON response"
        return 1
    }

    print_pass "POST /api/v1/lambda/invoke - Invoke function working"
    return 0
}

test_list_functions_endpoint() {
    print_test "GET /api/v1/lambda/functions - List Lambda functions"
    ((TESTS_RUN++))

    local response
    response=$(make_request GET "/api/v1/lambda/functions" "" 200) || {
        print_fail "GET /api/v1/lambda/functions - Request failed"
        return 1
    }

    validate_json "$response" "" "" || {
        print_fail "GET /api/v1/lambda/functions - Invalid JSON response"
        return 1
    }

    print_pass "GET /api/v1/lambda/functions - List functions working"
    return 0
}

test_get_function_info_endpoint() {
    print_test "GET /api/v1/lambda/functions/test-function - Get function info"
    ((TESTS_RUN++))

    local response
    response=$(make_request GET "/api/v1/lambda/functions/test-function" "" 200) || {
        print_fail "GET /api/v1/lambda/functions/test-function - Request failed"
        return 1
    }

    validate_json "$response" "" "" || {
        print_fail "GET /api/v1/lambda/functions/test-function - Invalid JSON response"
        return 1
    }

    print_pass "GET /api/v1/lambda/functions/test-function - Get function info working"
    return 0
}
...
main() {
    ...
    print_header "WaddleBot Lambda Action Module API Tests"
    ...
    if ! curl -s -f -o /dev/null "$LAMBDA_URL/health"; then
        print_error "Cannot connect to Lambda module at $LAMBDA_URL"
        print_error "Please ensure the Lambda module is running and accessible"
        exit 1
    fi
    echo -e "${GREEN}✓${NC} Lambda module is accessible"
    ...
    print_header "Lambda Action Endpoints"
