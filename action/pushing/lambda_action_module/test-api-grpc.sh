#!/bin/bash
# =============================================================================
# WaddleBot Lambda Action Module - gRPC API Tests
# Tests the gRPC interface for Lambda action functionality
# =============================================================================

set -e

# Configuration
GRPC_HOST="${GRPC_HOST:-localhost:50060}"
...
test_connectivity() {
    ...
    print_test "Describe LambdaAction service"
    if grpcurl -plaintext "$GRPC_HOST" describe lambda_action.LambdaAction 2>/dev/null; then
...
test_invoke_function() {
    ...
    local request='{"function_name":"test-function","payload":"{\"key\":\"value\"}","invocation_type":"RequestResponse"}'

    if [ "$VERBOSE" = "true" ]; then
        echo "Request: $request"
    fi

    local response=$(grpcurl -plaintext -d "$request" "$GRPC_HOST" lambda_action.LambdaAction/InvokeFunction 2>&1 || true)
...
test_list_functions() {
    ...
    local request='{"region":"us-east-1"}'

    local response=$(grpcurl -plaintext -d "$request" "$GRPC_HOST" lambda_action.LambdaAction/ListFunctions 2>&1 || true)
...
# Main
main() {
    echo -e "${BLUE}WaddleBot Lambda Action Module - gRPC API Tests${NC}"
    echo "Host: $GRPC_HOST"
