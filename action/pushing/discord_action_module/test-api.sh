#!/usr/bin/env bash
################################################################################
# WaddleBot Discord Action Module API Test Script
#
# Tests all REST API endpoints of the Discord Action Module with comprehensive
# validation, plus gRPC connectivity checks.
#
# Usage:
#   ./test-api.sh                       # Test default localhost:8000
#   DISCORD_URL=http://host:port ./test-api.sh
#   ./test-api.sh --help
#
# Exit codes:
#   0 - All tests passed
...
#   DISCORD_URL    - Base URL for Discord module (default: http://localhost:8000)
#   GRPC_PORT      - gRPC port for Discord module (default: 50051)
...
# Configuration
DISCORD_URL="${DISCORD_URL:-http://localhost:8000}"
...
EXAMPLES:
    ...
    DISCORD_URL=http://discord.example.com:8000 ./test-api.sh
...
make_request() {
    ...
    local url="${DISCORD_URL}${endpoint}"
    local curl_opts=(-s -w "\n%{http_code}" -X "$method")
...
test_health_endpoint() {
    ...
    validate_json "$response" ".module" "discord_action_module" || {
...
test_send_message_endpoint() {
    print_test "POST /api/v1/discord/messages - Send message"
    ((TESTS_RUN++))
...
EOF
    ...
    local response
    response=$(make_request POST "/api/v1/discord/messages" "$test_request" 200) || {
        print_fail "POST /api/v1/discord/messages - Request failed"
        return 1
    }

    validate_json "$response" "" "" || {
        print_fail "POST /api/v1/discord/messages - Invalid JSON response"
        return 1
    }

    print_pass "POST /api/v1/discord/messages - Send message working"
    return 0
}

test_send_embed_endpoint() {
    print_test "POST /api/v1/discord/embeds - Send embed"
    ((TESTS_RUN++))
...
EOF
    ...
    local response
    response=$(make_request POST "/api/v1/discord/embeds" "$test_request" 200) || {
        print_fail "POST /api/v1/discord/embeds - Request failed"
        return 1
    }

    validate_json "$response" "" "" || {
        print_fail "POST /api/v1/discord/embeds - Invalid JSON response"
        return 1
    }

    print_pass "POST /api/v1/discord/embeds - Send embed working"
    return 0
}

test_add_reaction_endpoint() {
    print_test "POST /api/v1/discord/reactions - Add reaction"
    ((TESTS_RUN++))
...
EOF
    ...
    local response
    response=$(make_request POST "/api/v1/discord/reactions" "$test_request" 200) || {
        print_fail "POST /api/v1/discord/reactions - Request failed"
        return 1
    }

    validate_json "$response" "" "" || {
        print_fail "POST /api/v1/discord/reactions - Invalid JSON response"
        return 1
    }

    print_pass "POST /api/v1/discord/reactions - Add reaction working"
    return 0
}
...
main() {
    ...
    print_header "WaddleBot Discord Action Module API Tests"
    ...
    if ! curl -s -f -o /dev/null "$DISCORD_URL/health"; then
        print_error "Cannot connect to Discord module at $DISCORD_URL"
        print_error "Please ensure the Discord module is running and accessible"
        exit 1
    fi
    echo -e "${GREEN}✓${NC} Discord module is accessible"
    ...
    print_header "Discord Action Endpoints"
    test_send_message_endpoint || true
    test_send_embed_endpoint || true
    test_add_reaction_endpoint || true
