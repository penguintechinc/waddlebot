#!/usr/bin/env bash
################################################################################
# WaddleBot Slack Action Module API Test Script
#
# Tests all REST API endpoints of the Slack Action Module with comprehensive
# validation, plus gRPC connectivity checks.
#
# Usage:
#   ./test-api.sh                       # Test default localhost:8000
#   SLACK_URL=http://host:port ./test-api.sh
#   ./test-api.sh --help
#
# Exit codes:
#   0 - All tests passed
...
#   SLACK_URL      - Base URL for Slack module (default: http://localhost:8000)
#   GRPC_PORT      - gRPC port for Slack module (default: 50052)
...
# Configuration
SLACK_URL="${SLACK_URL:-http://localhost:8000}"
...
EXAMPLES:
    ...
    SLACK_URL=http://slack.example.com:8000 ./test-api.sh
...
test_health_endpoint() {
    ...
    validate_json "$response" ".module" "slack_action_module" || {
...
test_send_message_endpoint() {
    print_test "POST /api/v1/slack/messages - Send message"
    ((TESTS_RUN++))
...
EOF
    ...
    local response
    response=$(make_request POST "/api/v1/slack/messages" "$test_request" 200) || {
        print_fail "POST /api/v1/slack/messages - Request failed"
        return 1
    }

    validate_json "$response" "" "" || {
        print_fail "POST /api/v1/slack/messages - Invalid JSON response"
        return 1
    }

    print_pass "POST /api/v1/slack/messages - Send message working"
    return 0
}

test_send_blocks_endpoint() {
    print_test "POST /api/v1/slack/blocks - Send message with blocks"
    ((TESTS_RUN++))
...
EOF
    ...
    local response
    response=$(make_request POST "/api/v1/slack/blocks" "$test_request" 200) || {
        print_fail "POST /api/v1/slack/blocks - Request failed"
        return 1
    }

    validate_json "$response" "" "" || {
        print_fail "POST /api/v1/slack/blocks - Invalid JSON response"
        return 1
    }

    print_pass "POST /api/v1/slack/blocks - Send blocks working"
    return 0
}

test_add_reaction_endpoint() {
    print_test "POST /api/v1/slack/reactions - Add reaction"
    ((TESTS_RUN++))
...
EOF
    ...
    local response
    response=$(make_request POST "/api/v1/slack/reactions" "$test_request" 200) || {
        print_fail "POST /api/v1/slack/reactions - Request failed"
        return 1
    }

    validate_json "$response" "" "" || {
        print_fail "POST /api/v1/slack/reactions - Invalid JSON response"
        return 1
    }

    print_pass "POST /api/v1/slack/reactions - Add reaction working"
    return 0
}
...
main() {
    ...
    print_header "WaddleBot Slack Action Module API Tests"
    ...
    if ! curl -s -f -o /dev/null "$SLACK_URL/health"; then
        print_error "Cannot connect to Slack module at $SLACK_URL"
        print_error "Please ensure the Slack module is running and accessible"
        exit 1
    fi
    echo -e "${GREEN}✓${NC} Slack module is accessible"
    ...
    print_header "Slack Action Endpoints"
    test_send_message_endpoint || true
    test_send_blocks_endpoint || true
