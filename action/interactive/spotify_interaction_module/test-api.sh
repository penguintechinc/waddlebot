#!/usr/bin/env bash
################################################################################
# WaddleBot Spotify Interaction Module API Test Script
################################################################################
#
# Comprehensive test suite for the Spotify Interaction Module API endpoints.
# Tests health checks, status, and module endpoints.
#
# Usage:
#   ./test-api.sh [OPTIONS]
#
...
#   --url URL           Set Spotify module URL (default: http://localhost:8026)
#   --api-key KEY       Set API key for authenticated endpoints
#   --verbose           Enable verbose output
#   --skip-auth         Skip tests requiring authentication
#
# Environment Variables:
#   SPOTIFY_URL         Base URL for Spotify module (default: http://localhost:8026)
#   SPOTIFY_API_KEY     API key for authenticated endpoints
...
# Configuration
SPOTIFY_URL="${SPOTIFY_URL:-http://localhost:8026}"
...
make_request() {
    ...
    local url="${SPOTIFY_URL}${endpoint}"
    local headers=(-H "Content-Type: application/json")

    # Add API key header if required and available
    if [[ "$auth_required" == "true" ]] && [[ -n "$SPOTIFY_API_KEY" ]]; then
        headers+=(-H "X-API-Key: ${SPOTIFY_API_KEY}")
    fi
...
run_test() {
    ...
    if [[ "$auth_required" == "true" ]] && [[ -z "$SPOTIFY_API_KEY" ]] && [[ "$SKIP_AUTH" == "true" ]]; then
...
main() {
    ...
    echo ""
    log_info "======================================================================"
    log_info "WaddleBot Spotify Interaction Module API Test Suite"
    log_info "======================================================================"
    log_info "Spotify Module URL: $SPOTIFY_URL"
    log_info "API Key: ${SPOTIFY_API_KEY:+[SET]}${SPOTIFY_API_KEY:-[NOT SET]}"
