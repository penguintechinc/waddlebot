#!/usr/bin/env bash
################################################################################
# WaddleBot Community Core Module API Test Script
################################################################################
#
# Comprehensive test suite for the Community Core Module API endpoints.
...
#   --url URL           Set community module URL (default: http://localhost:8020)
#   --api-key KEY       Set API key for authenticated endpoints
#   --verbose           Enable verbose output
#   --skip-auth         Skip tests requiring authentication
#
# Environment Variables:
#   COMMUNITY_URL       Base URL for community module (default: http://localhost:8020)
#   COMMUNITY_API_KEY   API key for authenticated endpoints
...
# Configuration
COMMUNITY_URL="${COMMUNITY_URL:-http://localhost:8020}"
...
make_request() {
    ...
    local url="${COMMUNITY_URL}${endpoint}"
    local headers=(-H "Content-Type: application/json")

    # Add API key header if required and available
    if [[ "$auth_required" == "true" ]] && [[ -n "$COMMUNITY_API_KEY" ]]; then
        headers+=(-H "X-API-Key: ${COMMUNITY_API_KEY}")
    fi
...
run_test() {
    ...
    if [[ "$auth_required" == "true" ]] && [[ -z "$COMMUNITY_API_KEY" ]] && [[ "$SKIP_AUTH" == "true" ]]; then
...
check_health_response() {
    ...
    # Verify module name
    local module_name
    module_name=$(echo "$response" | jq -r '.module')
    if [[ "$module_name" != "community_module" ]]; then
        log_error "Unexpected module name: $module_name (expected 'community_module')"
        return 1
    fi
...
check_healthz_response() {
    ...
    # Check for memory and cpu checks
    if ! echo "$response" | jq -e '.checks.memory' > /dev/null 2>&1; then
        log_error "Missing 'checks.memory' field"
        return 1
    fi

    if ! echo "$response" | jq -e '.checks.cpu' > /dev/null 2>&1; then
        log_error "Missing 'checks.cpu' field"
        return 1
    fi
...
check_metrics_response() {
    ...
    if ! echo "$response" | grep -q "waddlebot_memory_bytes"; then
        log_error "Missing 'waddlebot_memory_bytes' metric"
        return 1
    fi

    if ! echo "$response" | grep -q "waddlebot_cpu_percent"; then
        log_error "Missing 'waddlebot_cpu_percent' metric"
        return 1
    fi

    # Verify module name in metrics
    if ! echo "$response" | grep -q 'module="community_module"'; then
        log_error "Module name 'community_module' not found in metrics"
        return 1
    fi
...
check_status_response() {
    ...
    if ! echo "$response" | jq -e '.timestamp' > /dev/null 2>&1; then
        log_error "Missing 'timestamp' field"
        return 1
    fi
...
main() {
    ...
    echo ""
    log_info "======================================================================"
    log_info "WaddleBot Community Core Module API Test Suite"
    log_info "======================================================================"
    log_info "Community Module URL: $COMMUNITY_URL"
    log_info "API Key: ${COMMUNITY_API_KEY:+[SET]}${COMMUNITY_API_KEY:-[NOT SET]}"
