...
class _SettingsScreenState extends ConsumerState<SettingsScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return PopScope(
      // The "Settings saved"/"Settings save failed" SnackBar is raised
      // through the single ScaffoldMessenger MaterialApp installs above the
      // Navigator (app.dart), so it survives a pop back to Home and can
      // overlap the bottom-anchored Go Live button for its full ~4 s life,
      // swallowing a real tap. `canPop: false` + a manual pop here lets this
      // screen clear the messenger *before* the pop, for every dismissal
      // path (system back gesture, hardware back button, and the AppBar's
      // own back arrow all funnel through this callback).
      canPop: false,
                  ...
                  if (r != null) {
                    _update(
                      (GazerSettings s) => s.copyWith(
                        quality: s.quality.copyWith(resolution: r),
                      ),
                    );
                  }
                },
